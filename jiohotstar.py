...
class JioHotstar(BaseService):
	...
	ALIASES = ["JHS", "JioHotstar"]
	TITLE_RE = r"^https?://(?:www\.)?hotstar\.com/(?P<region>in|id|th)/(?P<type>shows|movies)/[a-z0-9-]+/(?P<id>\d+)"

	@staticmethod
	@click.command(name="JioHotstar", short_help="https://hotstar.com")
	@click.argument("title", type=str, required=False)
	@click.option("-rg", "--region", default="in", type=click.Choice(["in"], case_sensitive=False),
				  help="Account region")
	...
	def __init__(self, ctx, title, region, type):
		...
		self._cf_playback_url = None
		self._cf_domain = None
		self._hdntl_expiry = None
		self._cf_last_refresh = 0
		self._hdnea_content_id = None
		self._hdnea_range = None
		self._hdnea_expiry = None
		self._hdnea_last_refresh = 0
		self._TV_UA = "Disney+;in.startv.hotstar.dplus.tv/23.08.14.4.2915 (Android/13)"
	...
	def get_titles(self):
		...
		while True:
			try:
				...
				try:
					res_json = res.json()
					if "error" in res_json and res_json["error"].get("error_code") == "ERR_UM_USER_LOGGED_OUT":
						if not retried:
							self.log.warning(" + Detected logged out / invalid token cache (ERR_UM_USER_LOGGED_OUT). Invalidating token cache...")
							...
							if os.path.isfile(token_cache_path):
								try:
									...
									self.log.info(" + Successfully deleted poisoned token cache.")
								except Exception as ce:
									self.log.warning(f" - Failed to delete token cache file: {ce}")
							self.configure()
							headers["X-HS-UserToken"] = self.token
							retried = True
							continue
						else:
							raise ValueError(f"JioHotstar cookies / token are expired or invalid: {res_json['error'].get('error_message')}")
					data = res_json["success"]["page"]["spaces"]
					#self.log.debug(data)
				...
				if "cookies / token are expired" in str(e):
	...
	def get_playback(self, content_id, range_):
		if range_ in ("hdr10", "dv") or self.vcodec == "H265":
			video_code = "dvh265\",\"h265" if range_ == "dv" else "h265"
			quality = "4k"
		else:
			video_code = "h264"
			quality = "fhd"
	...
	def get_tracks(self, title):
		...
		if self.hdrdv:
			self.log.info(" + HYBRID mode: fetching HDR10 + DV manifests and merging...")
			tracks_hdr = self._get_tracks_for_range("hdr10")
			tracks_dv = self._get_tracks_for_range("dv")
			
			merged = Tracks()
			for track in tracks_hdr:
				merged.add(track)
			for track in tracks_dv:
				try:
					merged.add(track, warn_only=True)
				except TypeError:
					merged.add(track)
			for track in merged:
			...
			if self.range == 'HDR10':
				...
				range_ = 'dv'
			return self._get_tracks_for_range(range_)

	def _get_tracks_for_range(self, range_value: str) -> Tracks:
		playback = self.get_playback(self.content_id, range_value)
		...
		if 'vod-cf' in playback['content_url']:
			...
			data = datax.text
			self._apply_cf_cookies(datax.cookies.get_dict(), playback['content_url'])
			self._cf_playback_url = playback['content_url']
		else:
			mpd_url = playback['content_url']
			data_bytes, cookie = self._fetch_akamai_url(mpd_url)
			data = data_bytes.decode("utf-8") if isinstance(data_bytes, bytes) else data_bytes
			self.session.headers.update({
				"cookie": cookie,
				"user-agent": self._TV_UA,
			})
			self._log_hdnea_expiry(playback['content_url'])
			self._hdnea_content_id = self.content_id
			self._hdnea_range = range_value
		...
		for track in tracks:
			track.needs_proxy = True
			track.original_url = playback['content_url']

		for track in tracks.videos:
			if range_value == "hdr10":
				if not track.hdr10:
					track.hdr10 = True
	...
	def get_token(self):
		...
		if os.path.isfile(token_cache_path):
			...
			if token.get("exp", 0) > int(time.time()):
				...
				self.token_expiration = token.get("exp")
				return token["uid"]
			else:
				# expired, refresh
				self.log.info(" + Refreshing and using cached auth tokens...")
				uid = self.save_token(self.refresh(token["uid"], token["sub"]["deviceId"]), token_cache_path)
		else:
			# get new token
			if self.cookies:
				token_str = self.session.cookies.get("sessionUserUP", None, 'www.hotstar.com', '/' + self.region)
			else:
				raise self.log.exit(f" - Please add cookies")
			uid = self.save_token(token_str, token_cache_path)

		try:
			data = json.loads(base64.b64decode(uid.split(".")[1] + "===").decode("utf-8"))
			self.token_expiration = data.get("exp")
		except Exception:
			pass
		return uid
	...
	def ensure_authenticated(self) -> None:
		"""Re-authenticate if the token is near expiry, and refresh CDN cookies if needed."""
		if not self.token or not self.token_expiration:
			self.configure()
			return
		if int(time.time()) >= (self.token_expiration - 60):
			self.log.info(" + Token expiring soon, re-authenticating...")
			self.configure()
		self._refresh_cf_cookies()
		self._refresh_hdnea()

	def _apply_cf_cookies(self, cookies: dict, content_url: str) -> None:
		"""Parse and apply CloudFront cookies from a manifest response, extracting hdntl expiry."""
		self._cf_domain = urlparse(content_url).hostname or "hses3-vod-cf.cdn.hotstar.com"

		cf_cookie_map = {
			"hdntl": cookies.get("hdntl"),
			"CloudFront-Key-Pair-Id": cookies.get("CloudFront-Key-Pair-Id"),
			"CloudFront-Policy": cookies.get("CloudFront-Policy"),
			"CloudFront-Signature": cookies.get("CloudFront-Signature"),
		}

		hdntl_val = cf_cookie_map.get("hdntl") or ""
		try:
			exp_match = re.search(r"exp=(\d+)", hdntl_val)
			self._hdntl_expiry = int(exp_match.group(1)) if exp_match else None
		except Exception:
			self._hdntl_expiry = None

		if self._hdntl_expiry:
			expiry_dt = datetime.datetime.fromtimestamp(self._hdntl_expiry)
			mins_left = max(0, (self._hdntl_expiry - int(time.time())) // 60)
			self.log.info(
				f" + CloudFront hdntl valid until: {expiry_dt.strftime('%H:%M:%S')} "
				f"(expires in {mins_left} min)"
			)

		for name, value in cf_cookie_map.items():
			if value:
				self.session.cookies.set(name, value, domain=self._cf_domain, path="/")

		cookies_ = "; ".join(f"{k}={v}" for k, v in cf_cookie_map.items() if v)
		self.log.debug(f"Manifest Header -> Cookie: {cookies_}")
		self.session.headers.update({"cookie": cookies_})

	def _refresh_cf_cookies(self) -> None:
		"""Re-fetch the CloudFront manifest URL for fresh hdntl cookies if expiry is within 3 minutes. Throttled to once per 60s."""
		if not self._cf_playback_url:
			return

		now = int(time.time())

		if now - self._cf_last_refresh < 60:
			return

		if self._hdntl_expiry and now < (self._hdntl_expiry - 180):
			return

		self.log.info(" + CloudFront hdntl expiring soon, refreshing CDN cookies...")
		try:
			resp = self.session.get(self._cf_playback_url)

			if resp.status_code == 429:
				retry_after = int(resp.headers.get("Retry-After", 30))
				self.log.warning(
					f" + CDN rate limited (429) during cookie refresh. "
					f"Waiting {retry_after}s..."
				)
				time.sleep(retry_after)
				resp = self.session.get(self._cf_playback_url)

			if resp.ok and "<html" not in resp.text[:200]:
				self._apply_cf_cookies(resp.cookies.get_dict(), self._cf_playback_url)
				self._cf_last_refresh = int(time.time())
				self.log.info(" + CloudFront cookies refreshed successfully")
			else:
				self.log.warning(
					f" + CloudFront cookie refresh failed (status {resp.status_code}), "
					"will retry on next check"
				)
		except Exception as e:
			self.log.warning(f" + CloudFront cookie refresh error: {e}")

	def _log_hdnea_expiry(self, url: str) -> None:
		"""Parse hdnea token from URL and log its expiry time."""
		try:
			exp_match = re.search(r"hdnea=exp=(\d+)", url) or re.search(r"exp=(\d+)", url)
			if exp_match:
				self._hdnea_expiry = int(exp_match.group(1))
				expiry_dt = datetime.datetime.fromtimestamp(self._hdnea_expiry)
				mins_left = max(0, (self._hdnea_expiry - int(time.time())) // 60)
				self.log.debug(
					f" + hdnea token valid until: {expiry_dt.strftime('%H:%M:%S')} "
					f"(expires in {mins_left} min)"
				)
			else:
				self._hdnea_expiry = None
	...
	def _refresh_hdnea(self) -> None:
		"""Re-fetch playback for a fresh hdnea token if expiry is within 3 minutes. Throttled to once per 60s."""
		if not self._hdnea_content_id or not self._hdnea_range:
			return

		now = int(time.time())

		if now - self._hdnea_last_refresh < 60:
			return

		if self._hdnea_expiry and now < (self._hdnea_expiry - 180):
			return

		self.log.info(" + hdnea token expiring soon, refreshing...")
		try:
			playback = self.get_playback(self._hdnea_content_id, self._hdnea_range)
			new_url = playback.get("content_url", "")
			if new_url and "vod-cf" not in new_url:
				self._fetch_akamai_url(new_url)
				self._log_hdnea_expiry(new_url)
				self._hdnea_last_refresh = int(time.time())
				self.log.info(" + hdnea token refreshed successfully")
			else:
				self.log.warning(" + hdnea refresh skipped: URL switched to CloudFront or unavailable")
		except Exception as e:
			self.log.warning(f" + hdnea token refresh error: {e}")

	def _fetch_akamai_url(self, url: str) -> tuple[bytes, str]:
		ctx_ssl = ssl.create_default_context()
		ctx_ssl.check_hostname = False
		ctx_ssl.verify_mode = ssl.CERT_NONE
		req = Request(url)
		req.add_header("user-agent", self._TV_UA)
		r1 = urlopen(req, context=ctx_ssl)
		
		cookie_str = ""
		raw_cookies = r1.info().get_all("Set-Cookie")
		if raw_cookies:
			for c in raw_cookies:
				...
				if "=" in c_pair:
					...
					self.session.cookies.set(k_str, v_str, domain=domain, path="/")
		
		self.session.headers.update({"user-agent": self._TV_UA})
		return r1.read(), cookie_str
